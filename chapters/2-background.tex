...
Adaptive sampling has strong theoretical guarantees. In particular, it can be used to obtain a relative-error Frobenius norm approximation after sampling $\poly(k, 1/\epsilon)$ columns  \cite{deshpande_adaptive_2006}. However, the sampling distribution in adaptive sampling is expensive to compute directly as one must compute the squared norms of all residual columns at every step.
