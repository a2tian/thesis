...
% Let $r = \rank(\bM)$ and write $\bM = \bU\bSigma\bV^\top$ for the svd of $\bM$. The matrices $\bU, \bV\in\R^{n\times r}$ contain the left and right singular vectors as columns, respectively, and $\bSigma = \diag(\sigma_1, \sigma_2, \ldots, \sigma_r)$ where $\sigma_1\geq \sigma_2\geq\cdots \geq \sigma_r$ are the singular values of $\bM$. Define
% \[\bM_k := \bU\bSigma_k\bV^\top\]
% \todo{rewrite this}
% where $\bSigma_k\in \R^{r\times r}$ is the truncation of $\bSigma$ to the top-$k$ singular values: $\bSigma = \diag(\sigma_1,\ldots, \sigma_k, 0,\ldots, 0)$. Notice that $\bM_k$ has rank at most $k$.
...
% In our length-squared sampling algorithm, we use the following structural fact:
\begin{fact}
...
% Throughout, $\bA\in \R^{n\times d}$ denotes a real-valued matrix. If $\bA$ is psd and $S\subset [n]$ is a subset of indices of size $k$, we write
% \[\widetilde\bA = \bC\bW^{-1}\bC^\top\]
% to denote the \textit{Nystr\"om approximation} of $\bA$, where $\bC = \bA_{\ast, S}$ is the $n\times k$ submatrix of columns of $\bA$ indexed by $S$, and $\bW = \bA_{S, S}$ is the $k\times k$ principal submatrix of $\bA$ indexed by $S$.
