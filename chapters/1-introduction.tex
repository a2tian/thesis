...
where $\bA_{*, i}$ denotes the $i$th column of $\bA$. We show that for a psd matrix, we can draw such a sample without having to read all entries of the input. Our algorithm query $O(n)$ entries of $\bA$ and runs in $O(n)$ time in expectation. If the number of needed samples is small compared to $n$, our algorithm offers significant speedup over pre-computing all the column norms. 
