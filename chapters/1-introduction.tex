...
We also show that the query complexity of our algorithm is optimal up to constant factors. In particular, we prove an information-theoretic lower bound showing that any algorithm for length-squared sampling requires \(\Omega(n)\) entry queries in the worst case, even for psd matrices. 
...
% \todo{mention first sublinear algorithm for eigenvalue approximation}
% We then apply this sampling primitive to several problems in randomized matrix approximation. The applications focus on low-rank approximation of positive semidefinite matrices, including additive-error low-rank approximation, relative-error low-rank approximation, and robust low-rank approximation. We also discuss consequences for eigenvalue approximation. In each case, the role of the new sampler is to implement a length-squared column sampling step without first computing all column norms.
