...
A second open question is whether length-squared sampling can be used more directly for relative-error psd low-rank approximation. 

A third direction is to further develop robust sampling algorithms. Section~\ref{sec:robust-lra} showed that the rejection-sampling idea can be adapted to a corrupted input matrix after clipping. However, the robust algorithm assumes access to the diagonal corruption parameter $\phi_{\max}$ and has query complexity proportional to $\phi_{\max}^2$. It remains open whether this dependence can be improved or whether $\phi_{\max}$ can be estimated efficiently from entry queries.
