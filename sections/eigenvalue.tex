...
\begin{enumerate}[
    ...
    \[
        ...
        =
        \sqrt{\frac{\widehat F}{m\|\bA_{i_r,*}\|_2^2}}\,
        \be_{i_r}^{\top}
...
\(q_i=\|\bA_{i,*}\|_2^2/\|\bA\|_F^2\). Set
\[
    \alpha
    :=
    \frac{\epsilon}{4}\sqrt{\frac{\widehat F}{\|\bA\|_F^2}}.
\]
For each sampled index, Step 3 sets
\[
    \widehat q_{i_r}
    =
    \frac{\|\bA_{i_r,*}\|_2^2}{\widehat F}
    =
    \frac{\|\bA\|_F^2}{\widehat F}\,q_{i_r}.
\]
Therefore
\[
    \widehat \bS_{r,*}
...
\]
In particular, we may pick the constant $C$ sufficiently large compared to $c$ of Theorem~\ref{thm:sw-row-norm-eigenvalue} in order to apply the theorem with $m$ sampled indices and accuracy parameter $(\epsilon/4)\sqrt{\widehat F / \|\bA\|^2_F}$. It follows that, with probability at least
\(5/6\), if
\(
    \nu_1\geq\cdots\geq\nu_m
\)
