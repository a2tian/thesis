...
Bhattacharjee et al.~\cite{bhattacharjee_sublinear_2022}, who sample and rescale a small principal submatrix according to squared row norms, zero out certain entries to control variance, then return its eigenvalues together with additional zeros. Swartworth
...
\begin{theorem}[\cite{swartworth_tight_2024}, Theorem 7.8]
\label{thm:sw-row-norm-eigenvalue}
...
\[
    m_0 \geq
    \frac{c}{\epsilon^2}\log^4 n\log^2\frac{1}{\epsilon}
\]
and $c$ is the same absolute constant as in Definition~\ref{def:eigenvalue-zeroed-matrix}. Define the sampling
...
The preceding theorem assumes access to quantities that are not directly
...
\(\bA_{i_r,*}\) and compute
\(\|\bA_{i_r,*}\|_2^2\). In place of the exact sampling probability
\(q_{i_r}=\|\bA_{i_r,*}\|_2^2/\|\bA\|_F^2\), we use the estimated probability
\(\widehat q_{i_r}=\|\bA_{i_r,*}\|_2^2/\widehat F\). The algorithm
below is the resulting implementable version of Theorem~\ref{thm:sw-row-norm-eigenvalue}.
...
\begin{enumerate}[
    ...
    \)
    and, for each \(r\in[m]\), set
    \[
        \widehat q_{i_r}
        :=
        \frac{\|\bA_{i_r,*}\|_2^2}{\widehat F}.
    \]
    Define the sampling matrix \(\widehat \bS\in\R^{m\times n}\) by
    \[
        \widehat \bS_{r,*}
        :=
        \frac{\be_{i_r}^{\top}}{\sqrt{m\widehat q_{i_r}}}
        \qquad\text{for } r=1,\ldots,m.
    \]
    \item Compute the eigenvalues
    \(
        \widehat\lambda_1\geq \widehat\lambda_2\geq\cdots\geq \widehat\lambda_m
    \)
    of \(\widehat \bS\mathsf Z_{\epsilon/4,\widehat F}(\bA)\widehat \bS^\top\).
...
We first prove correctness. Condition on \(\widehat F\) produced by Step 1
of the algorithm and the indices \(i_1,\ldots,i_m\) sampled in Step 2. Let
\(\bS\) be the sampling matrix from Theorem~\ref{thm:sw-row-norm-eigenvalue}
formed using the same sampled indices and the exact sampling probabilities
\(q_i=\|\bA_{i,*}\|_2^2/\|\bA\|_F^2\), where
\[\bS_{r, *} = \frac{\be_{i_r}^\top}{\sqrt{mq_{i_r}}}.\]

Let
\(
    \alpha
    :=
    \frac{\epsilon}{4}\sqrt{\frac{\widehat F}{\|\bA\|_F^2}}.
\)
We make the following connection between \(\widehat \bS\mathsf Z_{\epsilon/4,\widehat F}(\bA)\widehat \bS^\top\) and $\bS\mathsf Z_{\alpha}(\bA) \bS^\top$.
First, 
\[
    \widehat \bS_{r,*}
    =
    ...
    \frac{\be_{i_r}^{\top}}{\sqrt{m q_{i_r}}}
    =
    \sqrt{\frac{\widehat F}{\|\bA\|_F^2}}\,\bS_{r,*},
\]
and hence \(\widehat \bS=\sqrt{\widehat F/\|\bA\|_F^2}\,\bS\). We also have
\(\mathsf Z_{\epsilon/4, \widehat F}(\bA)=\mathsf Z_{\alpha}(\bA)\) because
\[
...
Consequently,
\[
    \widehat \bS\mathsf Z_{\epsilon/4,\widehat F}(\bA)\widehat \bS^\top
    =
...
\[
    \frac{\epsilon}{8}\leq\alpha = \frac{\epsilon}{4}
    \sqrt{\frac{\widehat F}{\|\bA\|_F^2}}\leq\frac{\epsilon}{2}.
\]
In particular, we may pick the constant $C$ sufficiently large compared to $c$ of Theorem~\ref{thm:sw-row-norm-eigenvalue} in order to apply the theorem with $m$ sampled indices and accuracy parameter $\alpha$. It follows that, with probability at least
\(5/6\), if
\(
    \nu_1\geq\cdots\geq\nu_m
\)
...
\(\widehat \bS\mathsf Z_{\epsilon/4,\widehat F}(\bA)\widehat \bS^\top\),
again together with \(n-m\) additional zeros. Since
\(
    \widehat \bS\mathsf Z_{\epsilon/4,\widehat F}(\bA)\widehat \bS^\top
    =
...
the failure of Theorem~\ref{thm:sw-row-norm-eigenvalue}, the total success
...
In Step 4, we form the \(m\times m\) matrix
\(\widehat \bS\mathsf Z_{\epsilon/4,\widehat F}(\bA)\widehat \bS^\top\).
No additional entry queries are needed since every entry \(\bA_{i_r i_t}\)
is contained in the sampled row \(\bA_{i_r,*}\). Each entry can be computed
with constant arithmetic from \(\widehat F\), \(\widehat q_{i_r}\),
\(\widehat q_{i_t}\), and \(\bA_{i_r i_t}\), so forming this matrix costs
\(O(m^2)\) arithmetic operations. Since \(m<n\), this is bounded by
\(O(nm)\). Computing its eigenvalues costs \(O(m^3)\) arithmetic operations.
Step 5
