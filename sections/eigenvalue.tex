...
\begin{enumerate}[
    ...
    % \)
    % for each \(r\in[m]\).
    and define the sampling matrix \(\widehat \bS\in\R^{m\times n}\) by
    \[
        \bS_{r,*}'
        :=
        \sqrt{\frac{\widehat F}{m \|\bA_{i_r, *}\|^2_2}}\,
        \be_{i_r}^{\top}
        \qquad\text{for } r=1,\ldots,m.
    \]
    % Form the matrix \(\bG\in\R^{m\times m}\) entry-wise as given by
    % \[
    %     \bG_{rt}
    %     =
    ...
    \)
    of \(\bG:=\bS'^\top\mathsf Z_{\epsilon/4,\widehat F}(\bA)\bS'\).
