...
\(\bA_{i_r,*}\) and compute
\(\rho_r=\|\bA_{i_r,*}\|_2^2\). In place of the exact sampling probability
\(q_{i_r}=\rho_r/\|\bA\|_F^2\), we use the estimated probability
\(\widehat q_{i_r}=\rho_r/\widehat F\), so the rescaling factor is
\(1/\sqrt{m\widehat q_{i_r}}=\sqrt{\widehat F/(m\rho_r)}\). The algorithm
...
\]
In particular, we may pick the constant $C$ sufficiently large compared to $C_0$ of Theorem~\ref{thm:sw-row-norm-eigenvalue} in order to apply the theorem with $m$ sampled indices and accuracy parameter $(\epsilon/4)\sqrt{\widehat F / \|\bA\|^2_F}$. It follows that, with probability at least
\(5/6\), if
\(
    \nu_1\geq\cdots\geq\nu_m
\)
