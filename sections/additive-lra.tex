In this section, we derive a simple algorithm for computing additive error low-rank approximations to a psd matrix using our fast length-squared sampling procedure. Applying a result of Drineas, Kannan, and Mahoney~\cite{drineas_fast_2006-1}, we compute an approximate basis for the top $k$-dimensional left singular subspace of $\bA$ from $O(k/\epsilon^2)$ columns sampled by squared column norms. Then, we use a well-known leverage score sampling guarantee to approximately project $\bA$ onto this subspace. Given entry-wise access to $\bA\in\R^{n\times n}$, our algorithm outputs matrices $\bX, \bY\in\R^{n\times k}$ satisfying
...
Our sample complexity incurs an additional $1/\epsilon$ factor in comparison to the best known algorithm for additive error low-rank approximation: Bakshi, Chepurko, and Woodruff~\cite[Corollary 5.19]{bakshi_robust_2020} achieve an additive error low-rank approximation using only $\tilde O(nk/\epsilon)$ entries, where $\tilde O$ hides polylogarithmic factors in $n$, $k$, and $\epsilon$. Their work also achieves relative error approximation using $\tilde O(nk/\epsilon)$ entries, matching a $\Omega(nk/\epsilon)$ query lower bound of Musco and Woodruff~\cite{musco_sublinear_2019} up to polylogarithmic factors. The relative error guarantee from $\tilde O(nk/\epsilon)$ entries is a strictly stronger result from a theoretical perspective. Nonetheless, the algorithm presented here admits a simple analysis. We pose as an open question whether it is enough to sample only $O(k/\epsilon)$ columns by squared column norms, thus leading to an $O(nk/\epsilon)$ algorithm. The comparison is summarized in Table~\ref{tab:additive-lra-comparison}. 
% \todo{add remark about relative error approximation}
...
exponent, and \(\widetilde O\) hides polylogarithmic factors in $n$, $k$, and $1/\epsilon$.}
...
\label{thm:dkm06}
    ...
    Finally, let $\bH\in\R^{m\times k}$ be an orthonormal basis for the subspace spanned by the top-$k$ left singular vectors of $\bC$. We have the following:
    \begin{enumerate}[(a)]
...
With constant probability, Lemma~\ref{thm:dkm06} produces a matrix $\bH$ with orthonormal columns such that $\bH\bH^\top\bA$, the projection of $\bA$ onto the subspace spanned by $\bH$, yields an additive error approximation to $\bA$. To produce an explicit factorization, we would like to set $\bX = \bH$ and $\bY^\top = \bH^\top \bA$ so that $\bX\bY^\top$ yields an additive error approximation. However, computing $\bH^\top\bA$ requires us to read all of $\bA$, so we instead apply a standard matrix multiplication result to approximate $\bH^\top \bA$. We state and prove the needed guarantee below for completeness. A closely related result appears in Lemma 8 of \cite{drineas_fast_2006}.
% and Lemma 2.2.14 of \cite{musco_power_2018}.
...
\[
    q_i = \frac{\ell(i)}{k},\qquad \ell(i) = \|\bH_{i,*}\|^2_2.
\]
...
\begin{proof}
For a sample \(i\sim q\), define the random matrix
\[
    \bR=\frac{\bH_{i,*}^{\top}\bA_{i,*}}{q_{i}}.
\]
Then
\[
    \E[\bR]
    ...
    \bH^\top\bA.
\]
Let \(\bR_1,\ldots,\bR_t\) be the independent copies of \(\bR\) used to
form \(\widehat{\bZ}\). 
% Since the samples are independent and
% \(\E[\bR]=\bH^\top\bA\), we have
% \[
%     \E\|\widehat{\bZ}-\bH^\top\bA\|_F^2
%     =
%     \frac{1}{t}\E\|\bR-\bH^\top\bA\|_F^2
%     \leq
%     \frac{1}{t}\E\|\bR\|_F^2.
% \]
Since
...
\begin{aligned}
    ...
    &=
    \frac{1}{t^2}
    \sum_{r=1}^t\sum_{s=1}^t
    \E\operatorname{tr}\left[
        \left(\bR_r-\bH^\top\bA\right)^\top
        \left(\bR_s-\bH^\top\bA\right)
    \right].
\end{aligned}
\]
For \(r\neq s\), independence and
\(\E[\bR_r-\bH^\top\bA]=0\) imply
\(
    \E\operatorname{tr}\left[
        \left(\bR_r-\bH^\top\bA\right)^\top
        \left(\bR_s-\bH^\top\bA\right)
...
\begin{aligned}
    ...
    &=\sum_{i:\ell_i>0} k\|\bA_{i,*}\|_2^2
        \qquad\text{using \(q_i=\ell(i)/k\)}\\
    &\leq k\|\bA\|_F^2 .
\end{aligned}
\]
Thus,
...
\begin{enumerate}[
    ...
    \item Run Algorithm~\ref{alg:sampling-procedure} $s$ times on $\bA$, producing
    \(
        i_1,\ldots,i_s
    \)
    ...
    \(
        \bA_{*,i_1},\ldots,\bA_{*,i_s}
    \)
    and form the matrix \(\bB\in\R^{n\times s}\) whose \(r\)-th column is
    \[
        \bB_{*,r}
        =
...
\[
    s=\left\lceil \frac{Ck}{\epsilon^2}\right\rceil,
    \qquad
...
Let \(i_1,\ldots,i_s\) be the column indices sampled in Step 1 of
...
\begin{align*}
    \widetilde{\bC}_{*,r}
    &=
    \frac{1}{\sqrt{s p_{i_r}}}\bA_{*,i_r}\\
    &=\frac{\|\bA\|_F}{\sqrt s}
    \frac{\bA_{*,i_r}}{\|\bA_{*,i_r}\|_2}\qquad\text{since \(p_i=\|\bA_{*,i}\|_2^2/\|\bA\|_F^2\)}\\
    &=
    \frac{\|\bA\|_F}{\sqrt s}\bB_{*,r}.
\end{align*}
Thus
\(
    \widetilde{\bC}
    =
    \frac{\|\bA\|_F}{\sqrt s}\bB,
\)
...
It remains to bound the query complexity and time complexity. The \(s\) calls to
Algorithm~\ref{alg:sampling-procedure} use \(O(ns)\) entry queries in
expectation. Step 2 reads \(s\) full columns of \(\bA\), using \(ns\) further
...
For the arithmetic bound, forming and rescaling \(\bB\in\R^{n\times s}\) costs \(O(ns)\)
operations after the sampled columns have been read. To compute a top \(k\)
left singular subspace for \(\bB\) we compute \(\bB^\top\bB\in\R^{s\times s}\) and its svd, which requires
\(
    O(ns^2+s^3)
\)
arithmetic. For \(s\leq n\), this is \(O(ns^2)\), hence
\(
...
expected arithmetic cost of the sampling procedure contributes \(O(ns)\), which
is also dominated. Thus the total expected arithmetic complexity is
\(
    O\!\left({nk^2}/{\epsilon^4}\right).
\)
% If \(s>n\), then \(k/\epsilon^2=\Omega(n)\), and one may instead read all of
