...
\todo{remark on how to compute $\bH$}
...
For the arithmetic bound, forming and normalizing \(\bB\in\R^{n\times c}\) costs \(O(nc)\)
...
\)
Computing all leverage scores of \(\bH\) costs \(O(nk)\), and sampling from the
leverage score distribution can be done after an \(O(n)\)-time preprocessing
step. Finally, forming \(\widehat{\bY}\) from the \(t\) sampled rows costs
